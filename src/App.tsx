...
function App() {
  ...
  const [endpoint, setEndpoint] = useState(import.meta.env.VITE_WHEP_ENDPOINT_DEFAULT);
  const merchant = import.meta.env.VITE_DRM_MERCHANT;


  const generateTestToken = async () => {
    // Default test secret. In production, THIS MUST BE ON THE BACKEND.
    const secretKey = import.meta.env.VITE_DRM_SECRET;
    const secret = new TextEncoder().encode(secretKey);
    
    const optData = { merchant: merchant };
    const crt = [{ 
      assetId: 'poc', 
      storeLicense: true,
      profile: { purchase: {} },
    ...
    <div className="min-h-screen bg-gray-900 text-white p-8 flex flex-col items-center gap-8">
      ...
      <main className="w-full max-w-4xl">
        <Player 
          ...
          merchant={merchant}
          encrypted={encrypted}
          token={encrypted ? token : undefined}
        />
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button 
                  ...
                  className="text-xs bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 px-2 py-1 rounded border border-blue-500/30 transition-colors"
                >
                  Generate Test Token
                </button>
              </div>
              ...
              {encrypted && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                  <label htmlFor="token-input" className="block text-sm font-medium text-gray-400">
                    DRM Token (JWT)
                  </label>
                  <textarea
                    id="token-input"
                  ...
                  <p className="text-xs text-yellow-500/80 italic">
                    Note: Ensure your stream is encrypted with CastLabs-compatible metadata.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="p-6 bg-gray-800 rounded-lg border border-gray-700">
            ...
            <pre className="text-blue-400 text-sm overflow-x-auto whitespace-pre-wrap font-mono bg-black/30 p-4 rounded">
              {JSON.stringify({
                ...
                merchant,
                encrypted,
                token: encrypted ? (token ? `${token.substring(0, 10)}...` : undefined) : undefined
              }, null, 2)}
            </pre>
          </div>
        </div>
